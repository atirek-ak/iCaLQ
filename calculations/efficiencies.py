...
from calculations.helper import (getCrossSectionFromProcess,
                                 getEfficienciesFromProcess,
                                 getEfficienciesFromProcessAndTagNameTauTau,
                                 getImmediateSubdirectories,
                                 getNumbersFromCsvFiles, transposeMatrix)
from utilities.constants import (chirality_index, get_efficiency_prefix,
                                 global_data_precision, lepton_index,
                                 quark_index, tag_names)
...
def getEfficiencies(
    ...
    for index, coupling in enumerate(leptoquark_parameters.sorted_couplings):
        coupling_efficiency_directory_paths = [
            efficiency_directory_paths[0][index],
            efficiency_directory_paths[1][index],
            efficiency_directory_paths[2][index],
            efficiency_directory_paths[3][index],
            efficiency_directory_paths[4][index],
        ]
        # case tau tau
        if coupling[quark_index] == "3":
            coupling_to_process_efficiencies_map[coupling] = (
                readAndInterpolateEfficiencyTauTau(
                    coupling_efficiency_directory_paths, leptoquark_parameters
                )
            )
        else:
            coupling_to_process_efficiencies_map[coupling] = (
                readAndInterpolateEfficiency(
                    coupling_efficiency_directory_paths, leptoquark_parameters
                )
            )
...
def readAndInterpolateEfficiency(
    ...
    cross_terms_coupling: str = "",
    coupling1: str = "",
    coupling2: str = "",
    cross_terms: bool = False,
) -> Union[SingleCouplingEfficiency, CrossTermsEfficiency]:
    """
    ...
    for process_path in path_list:
        ...
        for bin_values in transposed_mass_values:
            def interpolation_function(m): return interp1d(
        ...
        if cross_terms:
            ...
            coupling1_cross_section = getCrossSectionFromProcess(
                process_path, coupling_to_process_cross_section_map, coupling1
            )
            coupling2_cross_section = getCrossSectionFromProcess(
                process_path, coupling_to_process_cross_section_map, coupling2
            )
            # get single couplings efficiencies
            coupling1_efficiencies = getEfficienciesFromProcess(
                process_path, coupling_to_process_efficiencies_map, coupling1
            )
            coupling2_efficiencies = getEfficienciesFromProcess(
                process_path, coupling_to_process_efficiencies_map, coupling2
            )
            for (
                interpolated_mass_value,
                coupling1_efficiency,
                coupling2_efficiency,
            ) in zip(
                interpolated_mass_values, coupling1_efficiencies, coupling2_efficiencies
            ):
                ...
                cross_terms_efficiency = round(
                    float(
                        (
                            coupling_to_process_cross_section_map[
                                cross_terms_coupling
                            ].actual_cross_section_tchannel
                            * interpolated_mass_value
...
def readAndInterpolateEfficiencyTauTau(
    ...
    coupling2: str = "",
    cross_terms: bool = False,
) -> Union[SingleCouplingEfficiencyTauTau, CrossTermsEfficiencyTauTau]:
    """
    ...
    for process_path in path_list:
        ...
        for tagName in tag_names:
            ...
            if cross_terms:
                ...
                # get single couplings efficiencies
                coupling1_efficiencies = getEfficienciesFromProcessAndTagNameTauTau(
                    process_path,
                    tagName,
                    coupling_to_process_efficiencies_map,
                    coupling1,
                )
                coupling2_efficiencies = getEfficienciesFromProcessAndTagNameTauTau(
                    process_path,
                    tagName,
                    coupling_to_process_efficiencies_map,
                    coupling2,
                ...
                for (
                    ...
                    interpolated_mass_values,
                    coupling1_efficiencies,
                    coupling2_efficiencies,
                ):
                    # we compute the cross-terms efficiency using the formula
                    # CS1 * EFF1 + CS2 * EFF2 + cross-terms-CS *  cross-terms-EFF = 2-couplings-CS * 2-couplings-EFF
                    cross_terms_efficiency = round(
                    ...
                    cross_terms_interpolated_values.append(
