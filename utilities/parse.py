...
def sortCouplingsAndValues(leptoquark_parameters: LeptoquarkParameters):
    """
    ...
    for line_number, coupling_value in enumerate(
        ...
        if len(coupling_value) != len(leptoquark_parameters.couplings):
            sys.exit(
                f"[Query error]: Coupling values length in line {line_number+1} is {coupling_value} which does not match the length of input couplings {leptoquark_parameters.couplings}"
            )
