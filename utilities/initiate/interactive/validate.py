...
def validateLeptoQuarkMass(
    ...
    try:
        ...
        if leptoquark_mass < minimum_leptoquark_mass or leptoquark_mass > maximum_leptoquark_mass:
            prRed(f"[Mass error]: Leptoquark mass should be from {minimum_leptoquark_mass} to {maximum_leptoquark_mass} GeV")
            return False
    except:
...
def validateLeptoQuarkCouplings(
    ...
    if not len(couplings_list):
        prRed("[Couplings error]: Couplings cannot be empty. For valid format, refer to README")
        return False
    for i in range(len(couplings_list)):
        if len(couplings_list[i]) != 10:
            prRed(f"[Couplings error]: The couplings input {couplings_list[i]} is not 10 characters. For valid format, refer to README")
            return False
        if not (
        ...
        if couplings_list[i][1:3] != "10":
            prRed(f"[Couplings error]: The second and third characters of {couplings_list[i]} should be '10'. For valid format, refer to README")
            return False
        if couplings_list[i][3] not in ["L", "R"]:
            prRed(f"[Couplings error]: The 4th character of {couplings_list[i]} should be either L or R for left-handed & right-handed couplings respectively. For valid format, refer to README")
            return False
        if couplings_list[i][4] not in ["L", "R"]:
            prRed(f"[Couplings error]: The 5th character of {couplings_list[i]} should be either L or R for left-handed & right-handed couplings respectively. For valid format, refer to README")
            return False
        if couplings_list[i][5] != '[':
            prRed(f"[Couplings error]: The 6th character of {couplings_list[i]} should be '['. For valid format, refer to README")
            return False
        if (leptoquark_model in scalar_leptoquark_models and couplings_list[i][6] not in ["1", "2"]) or (leptoquark_model in vector_leptoquark_models and couplings_list[i][6] not in ["1", "2", "3"]):
            prRed(f"[Couplings error]: The 7th character of {couplings_list[i]} should be a valid quark generation. For valid format, refer to README")
            return False
        if couplings_list[i][7] != ',':
            prRed(f"[Couplings error]: The 8th character of {couplings_list[i]} should be ','. For valid format, refer to README")
            return False
        if couplings_list[i][8] not in ["1", "2", "3"]:
            prRed(f"[Couplings error]: The 9th character of {couplings_list[i]} should be a valid lepton generation. For valid format, refer to README")
            return False
        if couplings_list[i][9] != ']':
            prRed(f"[Couplings error]: The 10th character of {couplings_list[i]} should be ']'. For valid format, refer to README")
            return False
    return True
...
def validateIgnoreSinglePairProduction(
    ...
    if ignore_single_pair_processes.lower() in {"yes", "y", "no", "n"}:
        return True
    prRed("[Ignore single pair production error]: ignore_single_pair takes input either 'yes'/'y' or 'no'/'n'")
    return False
...
def validateSignificance(
    ...
    try:
        ...
        if significance != 1 and significance != 2:
            prRed("[Significance error]: Significance should be a valid number: either 1 or 2")
            return False
    except:
        prRed("[Significance error]: Significance should be a valid number: either 1 or 2")
        return False
...
def validateSystematicError(
    ...
    try:
        ...
        if systematic_error < 0 or systematic_error > 1:
            prRed("[Systematic error]: Systematic error should be a valid number from 0 to 1.")
            return False
    except:
        prRed("[Systematic error]: Systematic error should be a valid number from 0 to 1.")
        return False
...
def validateExtraWidth(
    ...
    try:
        ...
        prRed("[Extra width error]: Extra Width constant should be a valid number")
        return False
    return True