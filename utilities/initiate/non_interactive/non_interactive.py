...
from utilities.constants import (InputMode, input_card_number_of_lines,
                                 luminosity, max_coupling_value_limit,
                                 min_coupling_value_limit)
from utilities.data_classes import (LeptoquarkParameters,
                                    NonInteractiveInputParameters)
from utilities.initiate.non_interactive.validate import \
...
def initiateNonInteractive(
    ...
    leptoquark_parameters, random_points = readCardData(
        non_interactive_input_parameters
    )
    # update random points in input vals file if required
    updateRandomPoints(
        random_points, non_interactive_input_parameters, leptoquark_parameters
    )
    # read input coupling values
    readInputCouplingValues(
        non_interactive_input_parameters, leptoquark_parameters)
...
def readCardData(
    ...
    if len(input_card_lines) != input_card_number_of_lines:
        sys.exit(
            f"Number of lines in file: {len(input_card_lines)}, expected {input_card_number_of_lines}. Please refer to README to check if all the data is present."
        )
    ...
    if random_points > 0:
        ...
        for _ in range(random_points):
            coupling_values_list = [
                str(random.uniform(min_coupling_value_limit, max_coupling_value_limit))
                for _ in range(len(leptoquark_parameters.couplings))
            ]
