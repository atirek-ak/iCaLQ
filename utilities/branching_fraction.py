...
def U1DecayWidthMassFactor(leptoquark_mass: float, mass_dictionary: List[float]):
    return (
        ...
        * absoluteeEfficiencyCouplingMassFactor(
            leptoquark_mass, mass_dictionary[0], mass_dictionary[1]
        )
    ...
    for coupling, symbolic_coupling in zip(
        ...
        if leptoquark_parameters.leptoquark_model == "U1":
            denominator += symbolic_coupling**2 * U1DecayWidthMassFactor(
                leptoquark_parameters.leptoquark_mass, mass_dictionary[coupling][0]
            )
            ...
            if coupling[chirality_index] == "L":
                denominator += symbolic_coupling**2 * U1DecayWidthMassFactor(
                    leptoquark_parameters.leptoquark_mass, mass_dictionary[coupling][1]
                )
