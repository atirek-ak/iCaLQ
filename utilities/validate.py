...
def validateInputData(
    ...
    if (
        ...
        raise ValueError(
            f"[Model error]: Not a valid leptoquark model. Allowed models: {scalar_leptoquark_models + vector_leptoquark_models}"
        )
    ...
    if (
        ...
        raise ValueError(
            f"[Mass error]: Leptoquark mass should be from {minimum_leptoquark_mass} to {maximum_leptoquark_mass} GeV"
        )
    ...
    for i in range(len(couplings_list)):
        if len(couplings_list[i].strip()) != 10:
            raise ValueError(
                f"[Couplings error]: The couplings input {couplings_list[i]} is not 10 characters. For valid format, refer to README"
            )
        if not (
            (
            ...
            raise ValueError(
                "[Couplings error]: For scalar leptoquarks, the first letter should be Y & for vector leptoquarks it should be X. For valid format, refer to README"
            )
        if couplings_list[i][1:3] != "10":
            raise ValueError(
                f"[Couplings error]: The second and third characters of {couplings_list[i]} should be '10'. For valid format, refer to README"
            )
        if couplings_list[i][3] not in ["L", "R"]:
            raise ValueError(
                f"[Couplings error]: The 4th character of {couplings_list[i]} should be either L or R for left-handed & right-handed couplings respectively. For valid format, refer to README"
            )
        if couplings_list[i][4] not in ["L", "R"]:
            raise ValueError(
                f"[Couplings error]: The 5th character of {couplings_list[i]} should be either L or R for left-handed & right-handed couplings respectively. For valid format, refer to README"
            )
        if couplings_list[i][5] != "[":
            raise ValueError(
                f"[Couplings error]: The 6th character of {couplings_list[i]} should be '['. For valid format, refer to README"
            )
        if (
        ...
        if couplings_list[i][7] != ",":
            raise ValueError(
                f"[Couplings error]: The 8th character of {couplings_list[i]} should be ','. For valid format, refer to README"
            )
        ...
        if couplings_list[i][9] != "]":
            raise ValueError(
                f"[Couplings error]: The 10th character of {couplings_list[i]} should be ']'. For valid format, refer to README"
            )
    ...
    if ignore_single_pair_processes.lower() in {"yes", "y", "true", "t", "1"}:
        ...
        raise ValueError(
            "[Ignore single pair production error]: ignore_single_pair takes input either 'yes'/'y' or 'no'/'n'"
        )
    ...
    try:
        ...
        if systematic_error < 0 or systematic_error > 1:
            raise ValueError(
                "[Systematic error]: [Systematic error should be a valid number from 0 to 1."
            )
    except:
        raise ValueError(
            "[Systematic error]: [Systematic error should be a valid number from 0 to 1."
