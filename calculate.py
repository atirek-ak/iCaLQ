...
def calculate(
    ...
    minima_list_1 = [
        ...
        for randarr in np.random.rand(6, len(leptoquark_parameters.couplings))
    ]
    # minima_list_2 = [
    #     optimize.minimize(lambda x: numpy_chisq(*flatten(x)), randarr)
    #     for randarr in np.random.rand(3, len(leptoquark_parameters.couplings))
    # ]
    ...
    print(*[f"\n{lambdastring[i]} : {opt_lambda_list[i]}" for i in range(len(leptoquark_parameters.couplings))])
    if interactive:
        ...
        while True:
            ...
            if lam_val_f.lower() in ["done", "exit"]:
                return
            if not lam_val_ok(lam_val_f, len(leptoquark_parameters.couplings)):
                prRed(f"[Query Error]: Please input {len(leptoquark_parameters.couplings)} float input/s.")
                # prRed(f"[Query Error]: Query values for lambdas are either not {len(leptoquark_parameters.couplings)} (number of lambdas) in count or not convertible to float.\n")
            ...
            delta_chisq, validity_list = get_delta_chisq(
                ...
                chisq_min,
                numpy_chisq,
                len(leptoquark_parameters.couplings),
                chi_sq_limits,
            )
    ...
    delta_chisq, validity_list = get_delta_chisq(
        lam_vals, original_lam_vals, chisq_min, numpy_chisq, len(leptoquark_parameters.couplings), chi_sq_limits
    )
