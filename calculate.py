...
def calculate(
    ...
    coupling_to_process_efficiencies_map = getEfficiencies(
        leptoquark_parameters, coupling_to_process_cross_section_map
    )

    # make symbolic couplings
    symbolic_couplings = [
        sym.Symbol(coupling) for coupling in leptoquark_parameters.sorted_couplings
    ]

    # get branching fraction
    mass_dictionary = makeLeptoquarkMassDictionary(
        leptoquark_parameters.sorted_couplings
    )
    branching_fraction = getBranchingFraction(
        leptoquark_parameters, symbolic_couplings, mass_dictionary
    )

    # calcualte chi-square
    chi_square_symbolic = getChiSquareSymbolic(
        leptoquark_parameters,
        branching_fraction,
        coupling_to_process_cross_section_map,
        coupling_to_process_efficiencies_map,
        symbolic_couplings,
        True,
    )
    # convert chi-square symbolic to numpy as it has faster execution
    numpy_chi_square_symbolic = lambdify(
        flatten(symbolic_couplings), chi_square_symbolic, modules="numpy"
    )

    numpy_chi_square_symbolic_zero_coupling = 0.0
    if len(leptoquark_parameters.sorted_couplings) > 1:
        chi_square_symbolic_zero_coupling = getChiSquareSymbolic(
            leptoquark_parameters,
            0.0,
            coupling_to_process_cross_section_map,
            coupling_to_process_efficiencies_map,
        ...
        numpy_chi_square_symbolic_zero_coupling = lambdify(
            flatten(symbolic_couplings),
            chi_square_symbolic_zero_coupling,
            modules="numpy",
        )
    ...
    minima_values_list = [
        ...
        for random_values_list in np.random.rand(
            5, len(leptoquark_parameters.sorted_couplings)
        )
    ]
    ...
    minima_values_list.extend(
        [
            ...
            for random_values_list in (
                np.random.rand(5, len(leptoquark_parameters.sorted_couplings))
                * number_of_minima_starting_points
            )
        ]
    )
    ...
    if len(leptoquark_parameters.sorted_couplings) > 1:
        ...
        all_zeros_coupling_values = [
            0.0 for x in leptoquark_parameters.sorted_couplings
        ]
        zero_minima = numpy_chi_square_symbolic_zero_coupling(
            *flatten(all_zeros_coupling_values)
        )
    ...
    print(
        *[
            f"\n{leptoquark_parameters.sorted_couplings[i]} : {chi_square_minima_couplings[i]}"
    ...
    if input_mode == InputMode.INTERACTIVE:
        ...
        while True:
            ...
            if coupling_values_input_interactive.lower() in [
                ...
                return
            if not validateInteractiveInputCouplingValues(
                coupling_values_input_interactive,
                len(leptoquark_parameters.sorted_couplings),
            ):
            ...
            delta_chi_square, validity_list = getDeltaChiSquare(
                leptoquark_parameters,
                [coupling_values_interactive],
                chi_square_minima,
                numpy_chi_square_symbolic,
                numpy_chi_square_symbolic_zero_coupling,
                branching_fraction,
            )
            print(
                f"Delta chi-square: {delta_chi_square[0]}\nAllowed: {validity_list[0]}"
            )
    ...
    delta_chi_square, validity_list = getDeltaChiSquare(
        leptoquark_parameters,
        leptoquark_parameters.sorted_couplings_values,
        chi_square_minima,
        numpy_chi_square_symbolic,
        numpy_chi_square_symbolic_zero_coupling,
        branching_fraction,
    )
    yes_list = [i for i in range(len(validity_list))
                if validity_list[i] == "Yes"]
    no_list = [i for i in range(len(validity_list))
               if validity_list[i] == "No"]

    # printing & outputting yes values
    print("\nYes List:")
    with open(
        non_interactive_input_parameters.output_yes_path, "w", encoding="utf8"
    ) as yes_file:
        for coupling in leptoquark_parameters.sorted_couplings:
            print(coupling, end="\t")
    ...
    print("\nNo List:")
    with open(
        non_interactive_input_parameters.output_no_path, "w", encoding="utf8"
    ) as no_file:
        for coupling in leptoquark_parameters.sorted_couplings:
            print(coupling, end="\t")
    ...
    print(
        f"Output files {non_interactive_input_parameters.output_yes_path} and {non_interactive_input_parameters.output_no_path} written"
    )

    # print common list
    with open(
        non_interactive_input_parameters.output_common_path, "w", encoding="utf8"
    ) as common_file:
        for coupling in leptoquark_parameters.sorted_couplings:
